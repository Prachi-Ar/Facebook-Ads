...
for (i in seq_along(fields_vector)) {
  ...
  query <- adlib_build_query(
    ...
    search_terms = ".",
    ad_delivery_date_min = "2022-04-11",
    ad_delivery_date_max = "2022-05-15",
...
fb_ad_list[['ad']] <- fb_ad_list[['ad']] %>%group_by(id)%>%
  mutate(ad_creative_bodies_u = paste(ad_creative_bodies, collapse = ""),
         ad_creative_link_captions_u = paste(ad_creative_link_captions, collapse = ""),
         ad_creative_link_descriptions_u = paste(ad_creative_link_descriptions, collapse = ""),
         ad_creative_link_titles_u = paste(ad_creative_link_titles, collapse = ""),
         languages_u = paste(languages, collapse = ""),
         publisher_platforms_u = paste(publisher_platforms, collapse = ""))
...
fb_ad_list[["demographic"]] <- fb_ad_list[["demographic"]] %>%
  mutate(age_gender = paste(fb_ad_list[["demographic"]]$age, "_", fb_ad_list[["demographic"]]$gender))
...
fb_ad_list[["region"]] <-  unnest(fb_ad_list[["region"]], cols = -id)
...
write.csv(fb_ad_list[["region"]], "region.csv")
#---------------------------------------------------------

for (i in colnames(merged_dataset)){
  if (class(merged_dataset[[i]] == "list")){
    unnest(merged_dataset, merged_dataset[[i]])
  }
}
